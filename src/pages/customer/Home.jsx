...
function Home() {
  ...
  const inputRefs = useRef({}); // untuk menyimpan ref input qty tiap produk
    ...
    <div className="min-h-screen p-4 rounded-2xl bg-gray-50 pb-24">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className='flex flex-col justify-center items-start'>
          ...
          <h1 className='text-2xl google-sans-flex-bold'>Hai, {localStorage.getItem('customer') ? JSON.parse(localStorage.getItem('customer')).name.split(' ')[0] : 'Kawan'}</h1>
          <p className='text-sm text-gray-500'>Mau pesen apa hari ini?</p>
        </div>
        <div>
          {/* logo */}
          <img src="web-app-manifest-192x192.png" alt="" srcSet="" width={100} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {products.map(p => (
          ...
          >
            ...
            <img src="https://i.ytimg.com/vi/CaqSlzAgkN4/maxresdefault.jpg" className='rounded-4xl object-cover relative bottom-10 h-24 shadow-md' alt="" srcSet="" />
            ...
            <h3 className="font-semibold google-sans-flex-bold text-left">{p.name}</h3>
            {/* Stok */}
            ...
            {getQty(p.id) === 0 ? (
              <div className="mt-4">
                <button onClick={() => addToCart(p)} className="bg-orange-500 text-white py-2 px-4 rounded-xl font-semibold shadow-lg">
                  Masuk Keranjang
                </button>
              </div>
            ) : (
              <div className="flex mt-4 flex-row justify-between items-center">
                <button  onClick={() => handleUpdateQty(p.id, getQty(p.id) - 1)} className="w-8 h-8 bg-gray-200 rounded-full">
                  -
                </button>

                <input type="number" value={getQty(p.id)} onChange={(e) => handleUpdateQty(p.id, parseInt(e.target.value) || 0)} className="font-bold text-center border border-gray-300 rounded-md w-10" />

                <button  onClick={() => handleUpdateQty(p.id, getQty(p.id) + 1)} className="w-8 h-8 bg-orange-500 text-white rounded-full">
                  +
                </button>
              </div>
            )}
